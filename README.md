...
**Frontend (`.env`):**
```bash
REACT_APP_BACKEND_URL=https://docpackhub.preview.emergentagent.com
```
...
### API Health Check
```bash
curl https://docpackhub.preview.emergentagent.com/api/health
```
...
# Submit intake
curl -X POST https://docpackhub.preview.emergentagent.com/api/intake/submit \
...
# Login
curl -X POST https://docpackhub.preview.emergentagent.com/api/auth/login \
  -H "Content-Type: application/json" \
