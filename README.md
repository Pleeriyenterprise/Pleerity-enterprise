...
**Frontend (`.env`):**
```bash
REACT_APP_BACKEND_URL=https://pleeritydocs.preview.emergentagent.com
```
...
### API Health Check
```bash
curl https://pleeritydocs.preview.emergentagent.com/api/health
```
...
# Submit intake
curl -X POST https://pleeritydocs.preview.emergentagent.com/api/intake/submit \
...
# Login
curl -X POST https://pleeritydocs.preview.emergentagent.com/api/auth/login \
  -H "Content-Type: application/json" \
