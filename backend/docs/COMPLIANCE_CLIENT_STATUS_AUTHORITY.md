...
| Property compliance tab (`GET /api/portfolio/properties/{property_id}/compliance-detail`) | `routes/portfolio.py`, `services/catalog_compliance.py:get_property_compliance_detail` | `KPI-authoritative` | **`kpis.status_valid`** (COMPLIANT/VALID) and **`kpis.lifecycle_satisfied_count`** (`is_requirement_satisfied`) are distinct; matrix row chips use lifecycle presentation | Yes (labelled) | Property Detail shows **Requirements satisfied** and **Valid for scoring** with distinct copy — `propertyDetailComplianceKpiPresentation.js` |
