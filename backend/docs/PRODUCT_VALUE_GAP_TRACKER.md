...
| **Linked decision record (workflow class)** | `backend/docs/REQUIREMENT_WORKFLOW_CLASS_DECISION_RECORD.md` — requirement → `client_workflow_class` inventory, open blockers (§5), and implementation gates before registry/resolver enforcement; **decision documentation only** until approved. **Domestic alarm completeness:** §2.6 (unified workflow, completeness vs scoring). |
| **Status** | Partial |
| **Completed work** | **Domestic alarm unified workflow (`smoke_heat_alarms`):** client and admin surfaces show **evidence completeness** (expected vs present components: smoke always; CO when context requires it) as **visibility/audit support** only — separate from top-level requirement/scoring state. Programme semantics: `REQUIREMENT_WORKFLOW_CLASS_DECISION_RECORD.md` §2.6. |
| **Remaining risks** | **Untagged documents:** completeness may treat uploads as satisfying smoke via **heuristic** filename/title matching, not structured component truth. **CO applicability:** inference from property/registry context still needs refinement so “why is CO shown / not shown?” stays aligned with legal expectation and support interpretation. |
