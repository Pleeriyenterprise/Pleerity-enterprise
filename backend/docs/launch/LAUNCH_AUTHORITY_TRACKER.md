...
| 3 | No misleading compliance wording remains | **PARTIAL** — improved; `PRESENTATION_LANGUAGE_GOVERNANCE.md` semantic families + `presentationLanguage.js` / vault formatters; ongoing copy review against authority docs. **Programme G (P1 Review Policy):** obligation status vocabulary governed by [`docs/governance/REVIEW_POLICY_VOCABULARY.md`](../../docs/governance/REVIEW_POLICY_VOCABULARY.md); staging closure per [`audit/p1_review_policy_signoff_01/CONSISTENCY_AUDIT.md`](../audit/p1_review_policy_signoff_01/CONSISTENCY_AUDIT.md) D1–D12 (target S3 UAT). Tracker: `CLOSED_LOOP_COMPLIANCE_ARCHITECTURE_TRACKER.md` § Programme G. **No launch gate change** — reference only. |
...
| G8 Tenant Operations | `ops_control_g8_tenant_operations` | VERIFIED_OPERATIONALLY | `20260525T025630Z` — bundle `ops_runtime_g8_tenant_operations_6fd5ac4c_d35a58ae` |

**Execution order:** G0 → G1 → G2 → G3 → G4 → G5 → G6 → G7 → G8 (sequential).

**Done when:** G0 + G1–G8 each `VERIFIED_OPERATIONALLY` or signed `WATCHLIST`; no open `TRUST_RISK_PRESENT` / `COGNITIVE_TRUST_RISK` / `PROJECTION_AUTHORITY_DRIFT` without remediation.
