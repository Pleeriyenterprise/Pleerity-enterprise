...
**Companion:** `CLOSED_LOOP_COMPLIANCE_ARCHITECTURE_TRACKER.md` (Stream E), `CLOSED_LOOP_ARCHITECTURAL_GAP_ANALYSIS.md`, `STREAM_F_PHASE2_CORRELATION_PROPAGATION.md` (correlation vs `score_change_log` / history).

**Last updated:** 2026-05-01 (row 9 standalone authority-sync enqueue; cross-cutting §6 correlation; outcome engine F2-B).
...
| 9 | Evidence authority sync (standalone) | `requirement_evidence_authority.sync_requirement_evidence_authority`; call sites: `routes/admin.py`, `routes/client_compliance_evidence.py`, scripts | Recompute `evidence_authority` blob | Y | Y (includes gap sync) | Partial | **Y `Enq`** on **admin document authority paths** + **client guided evidence** (`routes/admin.py` link/unlink/scope/evidence-match/backfill; `routes/client_compliance_evidence.py` create + verify) — **Stream B straggler micro-PR (2026-05-01)**; scripts unchanged | Partial | Partial (not all callers audit authority itself) | None by default | Residual until worker (same as other `Enq` paths) | Scripts / other call sites still require explicit `Enq` if they mutate authority |
...
6. **Correlation propagation vs score rows** — `recalculate_and_persist` passes `context["correlation_id"]` into **`score_ledger_events`** and **`COMPLIANCE_SCORE_UPDATED.metadata`**, and `job_runner` forwards queue job `correlation_id` into that context. **F2-A (2026-04-30):** when that context id is non-empty, the same value is stored on **`property_compliance_score_history`** and **`score_change_log`** inserts (optional field; no backfill). Callers that omit id keep prior behaviour (join via time/`reason`/property or ledger/audit). **Outcome engine (`apply_action_outcome`, F2-B):** supplies **`correlation_id`** in context (and on **`compliance_activity_log`**) via `_resolve_outcome_correlation_id`, which then reaches history/change_log via F2-A. Contract tests: `tests/test_stream_f_correlation_propagation_contract.py`, `tests/test_compliance_outcome_engine.py` (correlation assertions).
