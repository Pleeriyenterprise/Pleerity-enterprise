...
| 2026-05-24 | **PRELAUNCH-OPS-RUNTIME-VERIFY-02 G5 Documents OPERATIONAL_ORPHAN_STATE:** Wales HMO pilot — surface boot + document truth + real browser upload (hmo_license `be01b673…`, review=UPLOADED / op=EXTRACTION_FAILED, not instant verified) + convergence OK; **3 PROPERTY-scoped docs without requirement_id** + **1 doc linked to requirement not in runtime set** (`d2066cd2…`); bundle `ops_runtime_g5_documents_6fd5ac4c_d35a58ae` run `20260524T190110Z` — **G6 blocked** pending linkage remediation. |
