...
| 2026-05-23 | **PRELAUNCH-OPS-RUNTIME-VERIFY-01 F4 remediation (code):** regen merge/retention for operational debt signals; propagation lifecycle (`acknowledged`/`remediation_in_progress`); G9 issue replay idempotency; integration probe PASS (`regen_behaviour_evidence.json`). Staging operational rerun remains **`FAIL_OPERATIONAL`** until deploy + same-run browser proof. |
