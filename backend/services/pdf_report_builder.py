...
import io
from utils.branding import COMPANY_NAME, get_branding_website_url, SUPPORT_EMAIL, TAGLINE
...
def build_portfolio_report(client_id: str, report_data: dict) -> bytes:
    ...
    elements.append(Paragraph("Generated by " + COMPANY_NAME, styles["footer"]))
    elements.append(Paragraph(f"{get_branding_website_url()} | {SUPPORT_EMAIL}", styles["footer"]))
...
def build_score_explanation_report(
    ...
    # Footer
    elements.append(HRFlowable(width="100%", thickness=1, color=colors.gray, spaceAfter=12))
    elements.append(Paragraph(f"{COMPANY_NAME} – {TAGLINE}", styles["footer"]))
