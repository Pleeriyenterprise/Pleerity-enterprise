...
async def submit_quote_for_work_order(
    ...
    *,
    ...
    if st not in (PRICE_STATUS_AWAITING_QUOTE, PRICE_STATUS_REJECTED):
        raise ValueError("A quote can only be submitted when awaiting a quote or after a rejection")
    ...
    await create_audit_log(
        ...
        metadata={"pricing_event": "quote_submitted", "quoted_price": float(amount)},
    )
...
async def reject_quote_for_work_order(
    ...
    *,
    ...
    if not pricing_workflow_applies(wo):
        raise ValueError("Pricing workflow does not apply to this legacy job")
    if _norm_price_status(wo) != PRICE_STATUS_QUOTED:
        raise ValueError("Only a quoted price can be rejected")
    ...
    await db.work_orders.update_one(
        ...
        {
            "$set": {
                "price_status": PRICE_STATUS_REJECTED,
    ...
    await create_audit_log(
        ...
        resource_type="work_order",
        resource_id=work_order_id,
        metadata={"pricing_event": "quote_rejected", "reason": (reason or "")[:500]},
    )
    out = await _fetch_wo(work_order_id)
    if not out:
        raise RuntimeError("Work order missing after update")
...
def serialize_pricing_snapshot(wo: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for k in ("quote_submitted_at", "quote_approved_at", "quote_rejected_at", "inspection_completed_at"):
        v = out.get(k)
