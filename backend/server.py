...
async def lifespan(app: FastAPI):
    ...
    if os.environ.get("PYTEST_RUNNING") != "1":
        ...
        try:
            from services.lifecycle_aware_confirm_config import validate_lifecycle_confirm_boot
        ...
        try:
            from services.lifecycle_aware_scoring_config import validate_lifecycle_scoring_boot

            validate_lifecycle_scoring_boot()
        except Exception as e:
            logger.critical("Startup aborted (lifecycle scoring guard): %s", e)
            raise

        try:
            from services.lifecycle_aware_reminders_config import validate_lifecycle_reminder_boot

            validate_lifecycle_reminder_boot()
        except Exception as e:
            logger.critical("Startup aborted (lifecycle reminder guard): %s", e)
            raise

        try:
            from services.lifecycle_aware_kpis_config import validate_lifecycle_kpi_boot

            validate_lifecycle_kpi_boot()
        except Exception as e:
            logger.critical("Startup aborted (lifecycle KPI guard): %s", e)
            raise
...
app.include_router(admin_job_execution.router)
