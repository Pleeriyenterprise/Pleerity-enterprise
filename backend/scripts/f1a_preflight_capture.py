"""
F1a preflight: fixture gate + baseline (read-only). Writes f1a_* only; preserves f1_*.

  python -m scripts.f1a_preflight_capture --client-id CID --property-id PID
"""
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="F1a preflight (harness refinement — read-only)")
...
async def main() -> None:
    ...
    fixture_payload: Dict[str, Any] = {
        ...
        **resolved,
        ...
        "prior_f1_run": {
            "preserved": True,
            "verification_report": "f1_verification_report_6fd5ac4c_d35a58ae.json",
            "primary_rc_branch": "F1-RC-15",
            "reclassified_as": "harness_methodology_ack_replay_comparison",
        },
    }
    ...
    control_payload = {
        ...
        "control_client_id": ctrl_cid,
        "control_property_id": ctrl_pid,
        "selection_reason": "explicit_cli_default",
    ...
    _write(out_dir / f"f1a_fixture_classification_{slug}.json", fixture_payload)
    _write(out_dir / f"f1a_notification_before_{slug}.json", notification_before)
    _write(out_dir / f"f1a_control_selection_{slug}.json", control_payload)
