...
@admin_router.post("/test/abandoned-intake")
async def test_abandoned_intake_detection(
    current_user: dict = Depends(admin_route_guard),
):
    """
...
@admin_router.post("/test/high-intent-notification")
async def test_high_intent_notification(
    test_lead_id: str,
    current_user: dict = Depends(admin_route_guard),
):
    """
    Test HIGH intent notification by sending for an existing lead.
    """
    lead = await LeadService.get_lead(test_lead_id)
    if not lead:
        raise HTTPException(status_code=404, detail="Lead not found")
    
    await LeadService.notify_high_intent_lead(lead)
    
    return {
        "success": True,
        "message": f"HIGH intent notification sent for lead {test_lead_id}",
    }


@admin_router.post("/test/followup-queue")
async def test_followup_queue_processing(
    current_user: dict = Depends(admin_route_guard),
):
    """
    Manually trigger follow-up queue processing for testing.
    """
    await LeadFollowUpService.process_followup_queue()
    
    return {
        "success": True,
        "message": "Follow-up queue processing completed",
    }


@admin_router.post("/test/sla-check")
async def test_sla_check(
    sla_hours: int = 24,
    current_user: dict = Depends(admin_route_guard),
):
    """
    Manually trigger SLA breach check for testing.
    """
    from services.lead_followup_service import LeadSLAService
    
    breaches = await LeadSLAService.check_sla_breaches(sla_hours=sla_hours)
...
async def test_high_intent_notification(
    ...
    return {
        "success": True,
        "message": f"HIGH intent notification sent for lead {lead_id}",
    }
