"""Focused tests: DOCUMENT_UPLOAD / eicr client upload success → backbone observability (bounded slice)."""
import warnings
...
def test_workflow_activation_observability_filters_disallowed_targets_eicr_slice():
    ...
    fanout = {
        ...
        "downstream_trigger_targets": [
            {"downstream_target": "trust_surface.orchestrator.sync", "enqueue_outcome": "ENQUEUE_ACCEPTED"},
            {
...
@pytest.mark.parametrize(
    ...
    [
        ("eicr", "eicr", "eicr"),
        ("EICR", "electrical_installation", "eicr"),
        ("pat", "portable_appliance_test", None),
    ],
)
def test_eicr_bounded_slice_resolution(req_code, req_type, expected_slice):
...
def test_eicr_client_upload_success_response_shape_and_observability(client_http, tmp_path):
    user = {"client_id": "cli-eicr", "portal_user_id": "pu-eicr", "role": "ROLE_CLIENT_ADMIN"}
    pid, rid = "prop-eicr", "req-eicr"
    ...
    async def fake_sync(
        ...
        if transition_observability_out is not None:
            ...
            transition_observability_out.update(
                {
                    "transition_id": "tr:synthetic-eicr",
    ...
    with (
        ...
        patch.object(documents_routes, "evaluate_document_requirement_match", return_value=_match_eval_ok_eicr()),
        ...
        res = client_http.post(
            ...
            files={"file": ("eicr.pdf", b"%PDF-1.4 eicr", "application/pdf")},
            data={
                "property_id": pid,
                "requirement_id": rid,
                "document_type": "EICR",
    ...
    assert obs.get("obligation_slice") == "eicr"
...
def test_pat_upload_does_not_include_eicr_or_gas_observability_slice(client_http, tmp_path):
    user = {"client_id": "cli-pat2", "portal_user_id": "pu-pat2", "role": "ROLE_CLIENT_ADMIN"}
    pid, rid = "prop-pat2", "req-pat2"
    prop = {"property_id": pid, "client_id": user["client_id"], "is_active": True}
    req = {
        "requirement_id": rid,
        "client_id": user["client_id"],
    ...
    with (
        ...
        res = client_http.post(
            ...
            data={"property_id": pid, "requirement_id": rid, "document_type": "PAT"},
        )
...
def test_eicr_rst_backbone_blocked_shows_skip_observability(client_http, tmp_path):
    user = {"client_id": "cli-bk-e", "portal_user_id": "pu-bk-e", "role": "ROLE_CLIENT_ADMIN"}
    pid, rid = "prop-bk-e", "req-bk-e"
    prop = {"property_id": pid, "client_id": user["client_id"], "is_active": True}
    req_row = {
        "requirement_id": rid,
        "client_id": user["client_id"],
    ...
    with (
        ...
        res = client_http.post(
            "/api/documents/upload",
            files={"file": ("eicr.pdf", b"%PDF-1.4", "application/pdf")},
            data={"property_id": pid, "requirement_id": rid, "document_type": "EICR"},
        )
