...
def test_rent_payment_recorded_includes_amount_period_and_outstanding():
    ev = {
        ...
        "metadata": {
            ...
            "amount_minor": 40000,
            "period_key": "2026-10",
            "outstanding_balance_minor": 80000,
        },
    }
    row = ReportPresentationAuthority.present_timeline_row(ev)
    assert row["business_event"] == "Rent payment recorded"
    assert "£400.00" in row["summary"]
    assert "2026-10" in row["summary"]
    assert "£800.00" in row["summary"]
    assert "Oak City" in row["summary"]
