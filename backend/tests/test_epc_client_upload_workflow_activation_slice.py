"""Focused tests: DOCUMENT_UPLOAD / epc client upload success → backbone observability (bounded slice)."""
import warnings
...
def test_workflow_activation_observability_filters_disallowed_targets_epc_slice():
    from routes.documents import _workflow_activation_observability_for_bounded_document_upload_slice
    ...
    out = _workflow_activation_observability_for_bounded_document_upload_slice(
        fanout,
        obligation_slice="epc",
        document_upload_correlation_id="DOC_UPLOADED:doc1",
    )
    assert out["workflow_class"] == "DOCUMENT_UPLOAD"
    assert out["obligation_slice"] == "epc"
...
def test_epc_bounded_slice_resolution(req_code, req_type, expected_slice):
...
def test_epc_client_upload_success_response_shape_and_observability(client_http, tmp_path):
    user = {"client_id": "cli-epc", "portal_user_id": "pu-epc", "role": "ROLE_CLIENT_ADMIN"}
    pid, rid = "prop-epc", "req-epc"
    ...
    async def fake_sync(
        ...
        if transition_observability_out is not None:
            ...
            transition_observability_out.update(
                {
                    "transition_id": "tr:synthetic-epc",
    ...
    with (
        ...
        patch.object(documents_routes, "evaluate_document_requirement_match", return_value=_match_eval_ok_epc()),
        ...
        res = client_http.post(
            ...
            files={"file": ("epc.pdf", b"%PDF-1.4 epc", "application/pdf")},
            data={
                "property_id": pid,
                "requirement_id": rid,
                "document_type": "EPC",
    ...
    assert obs.get("obligation_slice") == "epc"
...
@pytest.mark.parametrize(
    "suffix,req_code,req_type,document_type,filename",
    [
        ("pat", "pat", "portable_appliance_test", "PAT", "pat.pdf"),
        ("leg", "legionella", "legionella", "Legionella risk assessment", "leg.pdf"),
    ],
)
def test_pat_and_legionella_upload_omit_bounded_observability(
    client_http, tmp_path, suffix, req_code, req_type, document_type, filename
):
    user = {"client_id": f"cli-{suffix}", "portal_user_id": f"pu-{suffix}", "role": "ROLE_CLIENT_ADMIN"}
    pid, rid = f"prop-{suffix}", f"req-{suffix}"
    prop = {"property_id": pid, "client_id": user["client_id"], "is_active": True}
    req = {
        "requirement_id": rid,
        "client_id": user["client_id"],
    ...
    with (
        ...
        res = client_http.post(
            ...
            data={"property_id": pid, "requirement_id": rid, "document_type": document_type},
        )
...
def test_epc_rst_backbone_blocked_shows_skip_observability(client_http, tmp_path):
    user = {"client_id": "cli-bk-p", "portal_user_id": "pu-bk-p", "role": "ROLE_CLIENT_ADMIN"}
    pid, rid = "prop-bk-p", "req-bk-p"
    prop = {"property_id": pid, "client_id": user["client_id"], "is_active": True}
    req_row = {
        "requirement_id": rid,
        "client_id": user["client_id"],
    ...
    with (
        ...
        res = client_http.post(
            "/api/documents/upload",
            files={"file": ("epc.pdf", b"%PDF-1.4", "application/pdf")},
            data={"property_id": pid, "requirement_id": rid, "document_type": "EPC"},
        )
