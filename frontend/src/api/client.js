...
apiClient.interceptors.response.use(
  ...
  (error) => {
    ...
    if (status === 403 || status === 429) {
      const errorCode =
        detail && typeof detail === 'object' && !Array.isArray(detail) ? detail.error_code : '';
      recordApiCircuitFailure(errPath, status, typeof message === 'string' ? message : '', errorCode);
    }
...
export const clientAPI = {
  ...
  getRequirementEvidenceResolution: (propertyId, requirementId) =>
    apiClient.get(
  ...
  complianceJobRequestVisitReschedule: (jobId, body) =>
    apiClient.post(`/jobs/${encodeURIComponent(jobId)}/request-visit-reschedule`, body ?? {}),
