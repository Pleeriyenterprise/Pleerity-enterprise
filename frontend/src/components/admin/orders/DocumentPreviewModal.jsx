...
/**
 * Format prompt_version_used for display (dict with template_id/version or string)
 */
const formatPromptVersion = (pv) => {
  if (pv == null) return '-';
  if (typeof pv === 'string') return pv;
  if (typeof pv === 'object' && pv !== null) {
    const tid = pv.template_id ?? pv.templateId ?? '';
    const ver = pv.version != null ? pv.version : '';
    return tid ? (ver !== '' ? `${tid} v${ver}` : tid) : JSON.stringify(pv);
  }
  return String(pv);
};
...
const DocumentMetadata = ({ version, orderId, serviceCode }) => {
  ...
  return (
    <div className="bg-gray-50 rounded-lg p-3 space-y-0.5">
      ...
      {version?.prompt_version_used != null && (
        <MetadataRow
          label="Prompt version"
          value={formatPromptVersion(version.prompt_version_used)}
          copyable
          onCopy={handleCopy}
