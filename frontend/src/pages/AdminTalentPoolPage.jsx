    ...
    <UnifiedAdminLayout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">Manage career applications</p>
        </div>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y">
                {loading ? (
                  ...
                  <tr><td colSpan="6" className="px-4 py-8 text-center text-gray-500">No submissions</td></tr>
                ) : (
                  submissions.map(sub => (
                    <tr key={sub.submission_id} className="hover:bg-gray-50">
                      <td className="px-4 py-3 text-sm">{new Date(sub.created_at).toLocaleDateString()}</td>
                      <td className="px-4 py-3 text-sm font-medium">{sub.full_name}</td>
                      <td className="px-4 py-3 text-sm">{sub.email}</td>
                      <td className="px-4 py-3 text-sm">{sub.country}</td>
                      <td className="px-4 py-3"><Badge className={getStatusColor(sub.status)}>{sub.status}</Badge></td>
                      <td className="px-4 py-3">
                        <Button size="sm" variant="outline" onClick={() => window.location.href = `/admin/talent-pool/${sub.submission_id}`}>
